...
def refresh_list():
    ...
    if len(rows) == 0:
        tk.Label(list_frame, text="Товары не найдены", bg=BG, font=FONT).pack(pady=20)
        return
    for row in rows:
...
def draw_card(parent, row):
    ...
    r = 1
    for line in lines:
        tk.Label(card, text=line, bg=bg, anchor="w", font=FONT).grid(row=r, column=1, sticky="w", padx=4)
        r = r + 1
    ...
    if d > 0:
        ...
        tk.Label(pf, text="Цена: %.2f" % p, bg=bg, font=FONT).pack(side=tk.LEFT)
...
def show_products():
    ...
    if role_is("Менеджер", "Администратор"):
        ...
        tk.Label(panel, text="Поиск:", bg=BG, font=FONT).grid(row=0, column=0, padx=4)
        search_var = tk.StringVar()
        search_var.trace_add("write", on_filter_change)
        tk.Entry(panel, textvariable=search_var, width=30, font=FONT).grid(row=0, column=1, padx=4)
        tk.Label(panel, text="Сортировка:", bg=BG, font=FONT).grid(row=0, column=2, padx=4)
        ...
        tk.Label(panel, text="Поставщик:", bg=BG, font=FONT).grid(row=1, column=0, padx=4)
    ...
    if role_is("Администратор"):
        btn(win, "Добавить товар", lambda: open_form("add", "")).pack(anchor="w", padx=12, pady=4)
...
def add_field(frame, row, label, key, value="", combo=None):
    tk.Label(frame, text=label, bg=BG, font=FONT).grid(row=row, column=0, sticky="w", pady=3)
    if combo is not None:
        w = ttk.Combobox(frame, values=combo, width=37)
        w.set(value)
    else:
        w = tk.Entry(frame, width=40, font=FONT)
        w.insert(0, value)
...
def open_form(mode, article):
    ...
    fw = tk.Toplevel(win)
    form_fields["win"] = fw
    fw.title("Добавление товара - ООО «Обувь»" if mode == "add" else "Редактирование товара - ООО «Обувь»")
    fw.geometry("650x700")
    fw.configure(bg=BG)
    fw.grab_set()
    ...
    ph.pack(pady=6)
    form_fields["ph"] = ph
    btn_plain(fw, "Выбрать фото", pick_photo).pack()
    ...
    if mode == "edit":
        ...
        if im:
            ph.config(image=im)
            ph.image = im
        tk.Label(fr, text="ID (артикул):", bg=BG, font=FONT).grid(row=0, column=0, sticky="w")
        e = tk.Entry(fr, width=40, font=FONT)
        e.insert(0, p[0])
...
def show_login():
    ...
    tk.Label(f, text="Логин:", bg=BG, font=FONT).pack()
    login_box = tk.Entry(f, width=40, font=FONT)
    login_box.pack(pady=4)
    tk.Label(f, text="Пароль:", bg=BG, font=FONT).pack()
    pass_box = tk.Entry(f, width=40, show="*", font=FONT)
